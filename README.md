...
resource "aws_vpc" "anuragpatilcloud_vpc" {
  cidr_block = "10.0.0.0/16"
  tags = {
    Name = "anuragpatilcloud-vpc"
  }
}
resource "aws_subnet" "anuragpatilcloud_subnet" {
  count = 2
  vpc_id                  = aws_vpc.anuragpatilcloud_vpc.id
  cidr_block              = cidrsubnet(aws_vpc.anuragpatilcloud_vpc.cidr_block, 8, count.index)
  availability_zone       = element(["ap-southeast-1a", "ap-southeast-1b"], count.index)
  map_public_ip_on_launch = true
  tags = {
    Name = "anuragpatilcloud-subnet-${count.index}"
  }
}
resource "aws_internet_gateway" "anuragpatilcloud_igw" {
  vpc_id = aws_vpc.anuragpatilcloud_vpc.id
  tags = {
    Name = "anuragpatilcloud-igw"
  }
}
resource "aws_route_table" "anuragpatilcloud_route_table" {
  vpc_id = aws_vpc.anuragpatilcloud_vpc.id
  route {
    cidr_block = "0.0.0.0/0"
    gateway_id = aws_internet_gateway.anuragpatilcloud_igw.id
  }
  tags = {
    Name = "anuragpatilcloud-route-table"
  }
}
resource "aws_route_table_association" "a" {
  count          = 2
  subnet_id      = aws_subnet.anuragpatilcloud_subnet[count.index].id
  route_table_id = aws_route_table.anuragpatilcloud_route_table.id
}
resource "aws_security_group" "anuragpatilcloud_cluster_sg" {
  vpc_id = aws_vpc.anuragpatilcloud_vpc.id
  egress {
    from_port   = 0
    to_port     = 0
  ...
  tags = {
    Name = "anuragpatilcloud-cluster-sg"
  }
}
resource "aws_security_group" "anuragpatilcloud_node_sg" {
  vpc_id = aws_vpc.anuragpatilcloud_vpc.id
  ingress {
    from_port   = 0
    to_port     = 0
  ...
  tags = {
    Name = "anuragpatilcloud-node-sg"
  }
}
resource "aws_eks_cluster" "anuragpatilcloud" {
  name     = "anuragpatilcloud-cluster"
  role_arn = aws_iam_role.anuragpatilcloud_cluster_role.arn
  vpc_config {
    subnet_ids         = aws_subnet.anuragpatilcloud_subnet[*].id
    security_group_ids = [aws_security_group.anuragpatilcloud_cluster_sg.id]
  }
}
resource "aws_eks_node_group" "anuragpatilcloud" {
  cluster_name    = aws_eks_cluster.anuragpatilcloud.name
  node_group_name = "anuragpatilcloud-node-group"
  node_role_arn   = aws_iam_role.anuragpatilcloud_node_group_role.arn
  subnet_ids      = aws_subnet.anuragpatilcloud_subnet[*].id
  scaling_config {
    desired_size = 3
    max_size     = 3
  ...
  remote_access {
    ...
    source_security_group_ids = [aws_security_group.anuragpatilcloud_node_sg.id]
  }
}
resource "aws_iam_role" "anuragpatilcloud_cluster_role" {
  name = "anuragpatilcloud-cluster-role"
...
resource "aws_iam_role_policy_attachment" "anuragpatilcloud_cluster_role_policy" {
  role       = aws_iam_role.anuragpatilcloud_cluster_role.name
  policy_arn = "arn:aws:iam::aws:policy/AmazonEKSClusterPolicy"
}
resource "aws_iam_role" "anuragpatilcloud_node_group_role" {
  name = "anuragpatilcloud-node-group-role"
  assume_role_policy = <<EOF
{
  "Version": "2012-10-17",
  "Statement": [
...
resource "aws_iam_role_policy_attachment" "anuragpatilcloud_node_group_role_policy" {
  role       = aws_iam_role.anuragpatilcloud_node_group_role.name
  policy_arn = "arn:aws:iam::aws:policy/AmazonEKSWorkerNodePolicy"
}
resource "aws_iam_role_policy_attachment" "anuragpatilcloud_node_group_cni_policy" {
  role       = aws_iam_role.anuragpatilcloud_node_group_role.name
  policy_arn = "arn:aws:iam::aws:policy/AmazonEKS_CNI_Policy"
}
resource "aws_iam_role_policy_attachment" "anuragpatilcloud_node_group_registry_policy" {
  role       = aws_iam_role.anuragpatilcloud_node_group_role.name
...
```plaintext
aws eks --region ap-southeast-1 update-kubeconfig --name anuragpatilcloud-cluster
```
...
kubectl create secret docker-registry regcred \
  ...
  --docker-username=anuragpatilcloud \
...
```plaintext
git branch: 'main', credentialsId: 'git-cred', url: 'https://github.com/AnuragPatil-cloud/FullStack-Blogging-App.git'
```
...
* **Select the added credential :** `your-email@gmail.com`(mail-cred)
...
This project was built and deployed by [**Anurag Patil**](https://github.com/AnuragPatil-cloud) 💡.
...
[![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/AnuragPatil-cloud) [![DockerHub](https://img.shields.io/badge/DockerHub-2496ED?style=for-the-badge&logo=docker&logoColor=white)](https://hub.docker.com/u/anuragpatilcloud)
